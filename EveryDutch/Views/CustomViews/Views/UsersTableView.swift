...
extension UsersTableView {
    ...
    // 셀 업데이트
    private func handleUpdate(_ indexPaths: [IndexPath]) {
    ...
    // 셀 insert
    private func handleAdd(_ indexPaths: [IndexPath]) {
    ...
    // 셀 삭제
    private func handleRemove(_ indexPaths: [IndexPath]) {
        guard self.viewModel.validateRowCountChange(
            currentRowCount: self.currentTableViewRowCount,
            changedUsersCount: -indexPaths.count
        ) else {
