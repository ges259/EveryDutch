...
final class IndexPathDataManager<T> {
    ...
    func dataChanged(_ userInfo: [String: Any]) {
        ...
        for (key, value) in userInfo {
            if let error = value as? ErrorEnum {
                ...
                self.indexPaths[key, default: []] = self.mergeIndexPaths(
                    indexPaths[key] ?? [],
                    newValues
                )
            }
    ...
    // Key 값만 반환
    func getKeys() -> [String] {
        return Array(self.indexPaths.keys)
    }
    
    // Section 값만 반환
    func getSections() -> [Int] {
        var sections: Set<Int> = []
        for indexPaths in self.indexPaths.values {
            sections.formUnion(indexPaths.map { $0.section })
        }
        return Array(sections).sorted()
    }
    
    // Row 값만 반환
    func getRows() -> [IndexPath] {
        return self.indexPaths.values.flatMap { $0 }
    }
}
