...
extension MainVC {
    ...
    private func handleUpdated(_ indexPaths: [IndexPath]) {
        guard self.viewModel.validateRowExistenceForUpdate(
            indexPaths: indexPaths,
            totalRows: self.currentTableViewRowCount
        ) else {
            self.collectionViewTotalReload()
            return
        }
        
        self.collectionView.reloadItems(at: indexPaths)
    }
    private func handleAdded(_ indexPaths: [IndexPath]) {
    ...
    private func handleRemoved(_ indexPaths: [IndexPath]) {
        guard self.viewModel.validateRowCountChange(
            currentRowCount: self.currentTableViewRowCount,
            changedUsersCount: -indexPaths.count
        ) else {
    ...
    // 셀 리로드
    func collectionViewTotalReload() {
