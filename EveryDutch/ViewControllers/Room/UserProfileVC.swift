...
final class UserProfileVC: UIViewController {
    ...
    var tableViewConstant: Constraint!
...
extension UserProfileVC {
    ...
    private func configureAutoLayout() {
        ...
        self.receiptTableView.snp.makeConstraints { make in
            self.tableViewConstant = make.height.lessThanOrEqualTo(0).constraint
        }
...
extension UserProfileVC {
    @objc private func searchBtnTapped() {
        if self.tableViewConstant.layoutConstraints.first?.constant == 0 {
            // 테이블 뷰가 숨겨져 있을 때 보이도록 설정하고 아래서 위로 애니메이션
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.tableViewConstant.update(offset: self.view.frame.height - self.cardHeight() - 150)
            ...
            self.tableViewConstant.update(offset: 0)
...
extension UserProfileVC {
    ...
    private func updateTableViewisHidden(_ bool: Bool = false) {
        ...
        if self.topViewBottomConstraint.layoutConstraints.first?.constant == 0
            || bool {
            print("\(#function) ----- 2")
            // 테이블 뷰가 숨겨져 있을 때 보이도록 설정하고 아래서 위로 애니메이션
            self.topViewBottomConstraint.update(offset: self.receiptTableView.frame.height)
            ...
            print("\(#function) ----- 3")
            // 테이블 뷰가 보이고 있을 때 위에서 아래로 애니메이션하고 숨김 설정
            self.topViewBottomConstraint.update(offset: 0)
